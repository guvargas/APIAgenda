...
routes.get('/compromissos/:idusuario/:idContato', (req, res) => {
    db.find({idcontato:req.params.idContato,
        idusuario:req.params.idusuario}).sort({}).exec((err, compromisso) => {
            if (err) {
...
routes.get('/compromissos/', (req, res) => {
    db.find({}).sort({}).exec((err, compromisso) => {
            if (err) {
                res.status(500).json({ error: err })
            } else {
                res.status(200).json({ compromisso });
...
routes.delete('/compromissos/:id', (req, res) => {
    db.remove({_id: req.params.id}, (err, compromisso) => {
            if (err) {
                res.status(500).json({ error: err })
            } else {
                res.status(200).json({deleted: compromisso});
            }
        }
    )
});
